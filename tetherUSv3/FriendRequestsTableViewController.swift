...
//  tetherUSv3
//
//  Created by Scott Kornblatt on 2/26/16.
...
class FriendRequestsTableViewController: UITableViewController {
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        dispatch_async(Utils.GlobalBackgroundQueue) {
            User(uid: self.user!.friendRequests![indexPath.row], callback: { (newuser) -> () in
    ...
    override func tableView(tableView: UITableView, editActionsForRowAtIndexPath indexPath: NSIndexPath) -> [UITableViewRowAction]? {
        ...
        let acceptAction = UITableViewRowAction(style: .Default, title: "Accept") { (action, indexPath) -> Void in
            
            User(uid: self.user!.friendRequests![indexPath.row], callback: { (newUser) -> () in
                
                UserUtils.AcceptFriend(self.user!, friend: newUser)
                self.tableView.reloadData()
            })
        }
        acceptAction.backgroundColor = UIColor.greenColor()
        
        let ignoreAction = UITableViewRowAction(style: .Default, title: "Ignore") { (action, indexPath) -> Void in
